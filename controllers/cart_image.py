...
def add_images_to_cart(images:AddImagesToCart, user: UserResponse = Depends(get_current_user_public), db: Session = Depends(get_db)):
    return CartImageService(db).add_images_to_cart(user.id, images.images_id)

@router.delete("/cart-images/{cart_image_id}")
def remove_cart_image(cart_image_id: int, user: UserResponse = Depends(get_current_user_public), db: Session = Depends(get_db)):
