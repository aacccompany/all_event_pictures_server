...
@router.post("/event/{event_id}/invite", response_model=EventUserResponse)
async def invite_event(event_id:int, user:UserResponse = Depends(get_current_user), db:Session = Depends(get_db)):
    return EventUserService(db).invite_event(event_id, user.id)
